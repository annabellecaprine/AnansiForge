...
(() => {
  ...
  async function compileLoreBookData() {
    const projName = projNameInput.value.trim() || 'Compiled Lorebook';
    const entries = [];
    let uidCounter = 1;
    ...
    for (const id of stagedIds) {
      ...
      if (catLower !== 'character' && catLower !== 'organization') continue;

      const nameClean = comp.name.split(' - ')[0].trim();
      const rawContent = contentOverrides[id] !== undefined ? contentOverrides[id] : (comp.content || '');
      const contentText = rawContent.trim();
      if (!contentText) continue;

      const keyWords = [nameClean];
      if (comp.lineage && comp.lineage.trim() && comp.lineage.trim().toLowerCase() !== nameClean.toLowerCase()) {
        keyWords.push(comp.lineage.trim());
      }
      if (Array.isArray(comp.tags)) {
        comp.tags.forEach(t => {
          const tClean = t.trim().replace(/^#/g, '');
          if (tClean && !keyWords.map(k => k.toLowerCase()).includes(tClean.toLowerCase()) && !/^[-\s]+$/.test(tClean)) {
            keyWords.push(tClean);
          }
        });
      }
      ...
      entries.push({
        ...
        key: keyWords,
        secondary_keys: [],
        comment: `${comp.category}: ${comp.name}`,
        content: contentText,
        constant: false,
        selective: false,
        selectiveLogic: 0,
    ...
    return {
      name: projName,
      description: `Lorebook generated by Anansi Forge for ${projName}`,
      scan_depth: 4,
      token_budget: 2048,
  ...
  async function exportAsLorebook() {
    try {
      const lorebook = await compileLoreBookData();
      if (!lorebook.entries || lorebook.entries.length === 0) {
        if (window.showToast) window.showToast('No staged Characters or Organizations found to export as Lorebook.', 'warning');
        return;
      }
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${lorebook.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}_lorebook.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      if (window.showToast) window.showToast(`Lorebook exported with ${lorebook.entries.length} entries.`, 'success');
    } catch (err) {
      console.error(err);
      if (window.showToast) window.showToast('Lorebook export failed', 'error');
    }
  }
