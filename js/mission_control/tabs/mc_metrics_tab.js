...
(() => {
    ...
    function renderMetrics() {
        ...
        const releases = S.state.includeArchived ? filteredBySeriesAll : activeReleases;
        ...
        const noMetrics = activeReleases.filter(r => S.isReleasePublished(r) && !hasMetrics(r));
      ...
      <div class="mc-include-archived-toggle">
        ...
        ${S.state.includeArchived ? '<span style="font-size:0.75rem; color:var(--text-muted); margin-left:8px;">KPIs, charts, and distribution include archived data</span>' : ''}
      </div>

      <div class="mc-kpi-grid" style="margin-bottom:20px;">
        ${kpiCard('\ud83d\udcac', totalMsgs.toLocaleString(), 'Total Messages' + (S.state.includeArchived ? ' (all)' : ''))}
        ${kpiCard('\ud83d\udc65', totalChats.toLocaleString(), 'Total Unique Chats' + (S.state.includeArchived ? ' (all)' : ''), 'var(--success)')}
        ${kpiCard('\u2b50', totalFavs.toLocaleString(), 'Total Favorites' + (S.state.includeArchived ? ' (all)' : ''), '#f59e0b')}
